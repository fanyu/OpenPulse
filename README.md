...
| **Codex** | Local SQLite (`~/.codex/state_5.sqlite`) | Sessions, token usage |
...
### Codex
No setup required. OpenPulse reads `~/.codex/state_5.sqlite` automatically.
...
| **Codex** | 本地 SQLite（`~/.codex/state_5.sqlite`）| 会话、Token 用量 |
...
### Codex
无需额外配置，OpenPulse 自动读取 `~/.codex/state_5.sqlite`。
